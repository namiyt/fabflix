...
<%
	out.println("<h3>"+request.getParameter("genre")+" Movies</h3>");
    ...
    ResultSet count = getCount.executeQuery("Select count(*) from movies where id in (select m.movie_id from genres_in_movies m inner join genres g on g.id=m.genre_id where g.name='"+request.getParameter("genre")+"')");
    ...
    ResultSet result = select.executeQuery("Select * from movies where id in (select m.movie_id from genres_in_movies m inner join genres g on g.id=m.genre_id where g.name='"+request.getParameter("genre")+"') limit " + (currentPage-1)*results + "," + results);

    out.print("Results per page: <form style='display:inline-block'><select name='n'><option value='10'>10</option><option value='25'>25</option><option value='50'>50</option></select><input type='submit' value='Go'><input type='hidden' name='genre' value='" + request.getParameter("genre") + "'><input type='hidden' name='page' value='1'></form><br>");

    out.print("Page " + currentPage + " of " + pages + "<br>");
    if (currentPage != 1) {
      out.print("<form style='display:inline-block'><button name='page' value='"+(currentPage-1)+"'>Prev</button><input type='hidden' name='genre' value='" + request.getParameter("genre") + "'><input type='hidden' name='n' value='" + request.getParameter("n") + "'></form>");
    }
    if (currentPage != pages) {
      out.print("<form style='display: inline-block'><button name='page' value='"+(currentPage+1)+"'>Next</button><input type='hidden' name='genre' value='" + request.getParameter("genre") + "'><input type='hidden' name='n' value='" + request.getParameter("n") + "'></form>");
    }
%>
